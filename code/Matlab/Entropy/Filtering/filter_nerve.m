function [fib, ind] = filter_nerve(filename, pixnerve, E, p, outfile)
    ...
    if isempty(p)
        ...
        p = 0:100;
        [fib, ~, M] = filter_fib_abs(nerve, pixnerve, E, p, true);   
    ...
    % Filters
    [fib, ind, M] = filter_fib_abs(nerve, pixnerve, E, p); 
   
    % Writes entropy scalars
    if isempty(outfile) == 0
        outfile
        outfile = strcat(outfile, 'Entropy_Weights.txt')
        fid = fopen(outfile, 'w');
        fid
        fprintf(fid,"%f ", M(:,2));
        fclose(fid);         
